...
app.post('/check-subscription', async (req, res) => {
  ...
  try {
    const subscription = await SubscribeModel.findOne({ email });
    ...
    if (subscription.expiryDate <= now) {
      ...
      return res.status(200).json({ message: 'Subscription expired. Profile updated to standard.' });
    }

    res.status(200).json({ message: 'Your subscription is active' });
  } catch (error) {
    console.error('Error checking subscription:', error);
    res.status(500).json({ message: 'Failed to check subscription. Please try again.' });
  }
});

app.post('/check-all-subscriptions', async (req, res) => {
  try {
    const now = new Date();
    const expiredSubscriptions = await SubscribeModel.find({ expiryDate: { $lte: now } });
    ...
    const updatePromises = expiredSubscriptions.map(subscription => {
      return RegisterModel.findOneAndUpdate(
        ...
        { profileType: 'standard' },
        { new: true }
      ).then(() => SubscribeModel.deleteOne({ email: subscription.email }));
    });

    await Promise.all(updatePromises);

    res.status(200).json({ message: 'All subscriptions have been updated.' });
  } catch (error) {
    console.error('Error processing all subscriptions:', error);
    res.status(500).json({ message: 'Failed to process all subscriptions. Please try again.' });
  }
});
...
app.post('/subscribe', async (req, res) => {
  ...
  try {
    ...
    const financeRecord = await FinanceModel.findOne();
    if (financeRecord) {
      financeRecord.total_amount += amount;
      await financeRecord.save();
    } else {
      const newFinanceRecord = new FinanceModel({ total_amount: amount });
...
app.post("/deleteVideo", async (req, res) => {
    ...
    try {
      ...
      // Deletes video record from the database
